...
export default async function AdminFormDetailsPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <main className="p-6">
      <AuthGate />
      <a className="text-gray-700 underline hover:text-gray-900" href={`/admin/forms?${usp.toString()}`}>← {tc('back_to_list')}</a>
      <h1 className="text-2xl font-semibold mb-4">{tadmin('form_details_title')}</h1>
        ...
        <div className="flex flex-col gap-3">
          ...
          <Card className="p-6">
            <div className="flex flex-col gap-3">
              ...
              <div className="text-base">{f.requirements?.applied ? tc('yes') : tc('no')}</div>
              {f.requirements?.link && (
                <div className="text-base">
                  <a href={String(f.requirements.link)} target="_blank" rel="noopener noreferrer" className="underline text-gray-700 hover:text-gray-900">{tadmin('requirements_link_label')}</a>
                </div>
              )}
            </div>
          </Card>
