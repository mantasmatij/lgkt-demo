...
export interface PillSelectProps {
  ...
  minWidth?: string; // default '20rem'
}

// Reusable pill-style select matching public form SelectField styling.
// Shows a scrollable list if options exceed maxVisible.
export function PillSelect({ id, label, value, onChange, options, placeholder, disabled, className = '', maxVisible = 5, autoWidth = true, minWidth = '20rem' }: PillSelectProps) {
  ...
  const dynamicStyle = autoWidth ? { width: `${approxCh}ch`, minWidth } : { minWidth };
  return (
    <div className={cn('flex flex-col gap-1', className)} style={dynamicStyle}>
      ...
      <Select
        ...
        size="md"
        classNames={{
          trigger: cn(inputClassNames.inputWrapper, `min-w-[${minWidth}]`),
